...
> **Note:** this page was substantially rewritten — see "Knockout bracket redesign…" below for the current behavior. The original version (described here) listed each round's finished matches with flags/scores/✓ but drew no bracket links.
- **Page**: `app/(app)/knockout/page.tsx` (force-dynamic, server component). Originally called `fetchWorldCupMatches()` (drops TBD fixtures), grouped by stage LAST_32 → LAST_16 → QUARTER_FINAL → SEMI_FINAL → THIRD_PLACE → FINAL, one card per round.
- **No new API or key**: reuses the existing `FOOTBALL_DATA_API_KEY` (football-data.org free tier).
...
## Knockout bracket redesign + glass nav + Vercel Analytics (added 2026-06)
This supersedes the nav order and `/knockout` rendering above. The bottom nav is now **Tournament · Knockout · [Home FAB] · Leaderboard · Profile**.

### `/knockout` rewritten as a live bracket (`app/(app)/knockout/page.tsx`)
- **`lib/football-api.ts` `fetchKnockoutSlots()`** (new): returns **all** knockout slots incl. not-yet-decided fixtures (`KnockoutSlot` = like `NormalizedMatch` but `homeTeam`/`awayTeam` can be `null`). This is what the bracket needs — `fetchWorldCupMatches()` drops TBD fixtures so it can't show future rounds. Display-only; never feeds scoring/sync.
- **Winner → next-match linkage is name-matched, never guessed.** `buildTeamSlotIndex()` maps team → the slots it appears in; `resolveNextMatch()` returns the slot in a strictly-later ladder stage that actually contains the winner. So a finished match's "advances to →" footer (`NextUp`) only shows a specific opponent **once the API has filled the next round's teams**. The free tier exposes **no bracket-linkage** (no `matchday`/`group`/link fields on KO matches) and only populates a next-round pairing once *both* feeder matches finish — and slowly. **Decision: do NOT hardcode the bracket tree** — this is football-data's *simulated* dataset with no verifiable slot mapping, a wrong guess shows wrong matchups (a bug the user explicitly rejected), and a guess could later contradict the API once it fills the teams. So next-round matchups appear dynamically only when the API provides them.
- **`components/KnockoutAutoRefresh.tsx`** (new, client): calls `router.refresh()` every 45s **only while a KO match is in the live window** (`isLiveWindow` from `lib/time.ts`), pauses when the tab is hidden, refreshes on focus regain, tears down on unmount. Gated server-side (`liveActive = allSlots.some(s => s.status !== "FINISHED" && isLiveWindow(s.kickoffAt))`) so it never polls the free tier on quiet days. This is what makes a just-finished result / newly-published next pairing appear without a manual reload.
- **RTL score fix:** the center score is printed `dir="ltr"` as **away–home** (not home–away), because in the RTL grid the home team renders on the right; printing home–away under forced LTR mirrored the digits relative to the teams (made it read as if the loser won). The shared `components/MatchCard.tsx` did **not** have this bug — it lets the score flex flow naturally RTL, so don't "fix" it there.
- **Winner styling is intentionally minimal:** winner = bold green name + a single green `Check` icon (no flag ring/pill/glow — the user asked to keep it un-noisy). Loser = dimmed/grayscale flag + muted strike-through name. New i18n under `t.knockout` (`advancesTo`, `advanced`, `tbdTeam`, `vsLabel`, …) and stage names in `t.stage`.

### Bottom nav: liquid-glass + Home in center (`components/BottomNav.tsx`)
- Floating frosted-glass pill (translucent `bg-surface/55` + `backdrop-blur-2xl backdrop-saturate-150`, top-edge highlight, soft shadow) instead of the old full-width bar. Active tab gets a soft inset glass highlight.
