...
/**
 * One-off: tell Telegram users who have NOT joined the prize tournament that it
 * is about to start (kickoff tonight, Belgium–Iran) and to come join before the
 * whistle. Recipients = users with a telegram_id absent from tournament_members.
 *
 * Defaults to DRY-RUN (prints recipients + the message, sends nothing).
 * Pass --send to actually deliver. Always dry-run first.
 *
 *   npx tsx scripts/remind-tournament-starting-soon.ts          # dry-run
 *   npx tsx scripts/remind-tournament-starting-soon.ts --send   # really send
 */
import fs from "fs";
...
function buildMessage(appUrl: string): string {
  return [
    "سلام 👋",
    "",
    "⏰ تورنومنت ویژهٔ جام جهانی ۲۰۲۶ در ۱ای‌بت همین امشب شروع می‌شود!",
    "",
    "فقط چند ساعت تا سوت آغاز بازی بلژیک–ایران مونده و درست از همین بازی، جدول تورنومنت رسماً کلید می‌خوره.",
    "",
    "🏆 ورودی هر نفر ۱۰۰٬۰۰۰ تومان و کل جایزه (مجموع ورودی همهٔ شرکت‌کننده‌ها) به نفر اول می‌رسد.",
    "",
    "اگه هنوز ثبت‌نام نکردی، همین حالا بیا تا جا نمونی 👇",
    `👉 ${appUrl}/tournament`,
    "",
...
async function main() {
  ...
  // Belt-and-suspenders filter. With --all, target every telegram user;
  // otherwise exclude users already in tournament_members.
  const memberSet = new Set(memberIds);
  const targets = recipients.filter((u) => u.tg && (ALL || !memberSet.has(u.id)));

  const message = buildMessage(APP_URL);

  console.log(`Mode: ${SEND ? "SEND (live)" : "DRY-RUN (nothing sent)"}`);
  console.log(
    `Recipients (${ALL ? "ALL telegram users" : "telegram users not in tournament"}): ${targets.length}`,
  );
  console.log("------------------------------------------------------------");
